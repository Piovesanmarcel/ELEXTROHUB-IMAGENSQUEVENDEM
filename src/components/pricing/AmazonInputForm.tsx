...
interface AmazonInputFormProps {
  inputs: AmazonPricingInputs;
  onInputChange: (field: keyof AmazonPricingInputs, value: string | boolean) => void;
  isBreakeven: boolean;
}

const AmazonInputForm = ({ inputs, onInputChange, isBreakeven }: AmazonInputFormProps) => {
  return (
    <Card className="glass-effect">
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center gap-2 text-lg">
          <Truck className="h-5 w-5 text-orange-600" />
          Dados do Produto - Amazon
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Label htmlFor="free-shipping">Amazon Prime (Frete Grátis)</Label>
          <Switch
            id="free-shipping"
...
export default AmazonInputForm;
