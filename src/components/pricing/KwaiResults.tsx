...
import { KwaiPricingResults, KwaiPricingInputs } from "@/types/kwai-pricing";

interface KwaiResultsProps {
  results: KwaiPricingResults;
  inputs: KwaiPricingInputs;
  isBreakeven: boolean;
}

const KwaiResults = ({ results, inputs, isBreakeven }: KwaiResultsProps) => {
    ...
    <Card className="glass-effect">
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          Resultados da Precificação - Kwai
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-yellow-50 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-yellow-800">Preço Final</span>
            </div>
            <p className="text-2xl font-bold text-yellow-900">{formatCurrency(results.finalPrice)}</p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
            <span className="text-sm text-gray-600">Comissão Kwai (10%)</span>
          ...
          <div className="flex justify-between items-center p-3 bg-yellow-100 rounded-lg border border-yellow-200">
            <span className="text-sm font-medium text-yellow-800">Custos Totais</span>
            <span className="font-bold text-yellow-900">{formatCurrency(results.totalCosts)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
