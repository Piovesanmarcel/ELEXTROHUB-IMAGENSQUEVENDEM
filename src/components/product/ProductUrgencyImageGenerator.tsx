...
interface ProductUrgencyImageGeneratorProps {
  productId: string;
  productName: string;
  productSku: string;
  images: string[];
...
export const ProductUrgencyImageGenerator = ({ 
  productId, 
  productName, 
  productSku, 
  images
}: ProductUrgencyImageGeneratorProps) => {
  const [selectedImageIndex, setSelectedImageIndex] = useState(0);
  const [urgencyText, setUrgencyText] = useState<string>('');
  ...
  const loadUrgencyFromCopywriting = async () => {
    try {
      ...
      if (data?.results && typeof data.results === 'object' && 'copywriting' in data.results) {
        ...
        const extractedUrgency = extractUrgencyFromCopywriting(copywriting);
        setUrgencyText(extractedUrgency);
      }
  ...
  const extractUrgencyFromCopywriting = (copywriting: string): string => {
    const lines = copywriting.split('\n');
    let inUrgencySection = false;
    let urgencyText = '';
    
    for (const line of lines) {
      const trimmedLine = line.trim();
      
      // Detectar início da seção de gatilho de escassez e urgência
      if (trimmedLine.includes('6.') && 
          (trimmedLine.toLowerCase().includes('gatilho') || 
           trimmedLine.toLowerCase().includes('escassez') ||
           trimmedLine.toLowerCase().includes('urgência'))) {
        inUrgencySection = true;
        continue;
      }
      
      // Detectar fim da seção (próxima seção numerada ou markdown headers)
      if (inUrgencySection && (trimmedLine.match(/^\d+\./) || trimmedLine.match(/^#{1,6}\s/))) {
        break;
      }
      
      // Extrair apenas o texto de urgência, ignorando formatações
      if (inUrgencySection && trimmedLine && 
          !trimmedLine.match(/^[-*]\s/) && 
        ...
        if (cleanText && cleanText.length > 0) {
          urgencyText += cleanText + ' ';
        }
      }
    }
    
    return urgencyText.trim();
  };
  ...
  const generateImageWithCanvas = async (): Promise<string> => {
    ...
    try {
      ...
      console.log('🎨 Desenhando caixa de urgência...');
      ...
      // Borda da caixa (vermelho)
      ctx.strokeStyle = 'rgba(239, 68, 68, 0.5)'; // red com transparência
      ctx.lineWidth = 2;
      ctx.strokeRect(boxX, boxY, boxWidth, boxHeight);
      console.log('✅ Caixa de urgência desenhada');

      // Desenhar texto de urgência
      console.log('🎨 Desenhando texto de urgência...');
      ...
      console.log('✅ Urgência desenhada');
  ...
  const generateImage = async () => {
    ...
    if (!urgencyText || urgencyText.length === 0) {
      toast.error('Nenhum gatilho de escassez e urgência encontrado no copywriting');
      return;
    }
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      const blob = await response.blob();
      const fileName = `${productName.replace(/[^a-zA-Z0-9]/g, '_')}_urgencia`;
      ...
      <Card className="glass-effect border-2 border-red-200">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-red-700">
            ...
            Gerador de Imagem com Gatilho de Escassez e Urgência
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="glass-effect border-2 border-red-200">
        ...
        <CardContent>
          <div className="text-center py-6">
            <Type className="h-12 w-12 text-red-300 mx-auto mb-3" />
            <p className="text-muted-foreground mb-2">
              Nenhum gatilho de escassez e urgência encontrado no copywriting gerado
            </p>
            <p className="text-sm text-muted-foreground">
    ...
    <Card className="glass-effect border-2 border-red-200">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-red-700">Gerador de Imagem com Gatilho de Escassez e Urgência</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-3 bg-red-50 rounded-lg border border-red-200">
          <p className="text-sm font-medium text-red-700 mb-2">Gatilho de escassez e urgência que será aplicado:</p>
        ...
        {showPreview && generatedImage && (
          <div className="mt-4">
            ...
            <div className="border rounded-lg overflow-hidden">
              <img
                src={generatedImage}
                alt="Imagem gerada com gatilho de escassez e urgência"
