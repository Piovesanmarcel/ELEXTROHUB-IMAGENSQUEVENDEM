...
import { SheinPricingInputs, SheinPricingResults } from "@/types/shein-pricing";

export const calculateSheinPricing = (inputs: SheinPricingInputs, commissionRate: number = 0.175): SheinPricingResults => {
  ...
  // Taxas da Shein
  const fixedFee = 3.00; // R$ 3,00 por item
...
export const calculateSheinBreakeven = (inputs: SheinPricingInputs, commissionRate: number = 0.175): SheinPricingResults => {
  const cost = parseFloat(inputs.costPrice) || 0;
  const tax = parseFloat(inputs.taxRate) || 0;
  const shippingCost = parseFloat(inputs.estimatedShippingCost) || 0;
