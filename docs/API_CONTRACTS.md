...
```sql
-- Chamada autenticada (extrai user_id do JWT via auth.uid())
SELECT debit_generation_credit(1);

-- Nota: Requer sessão autenticada (JWT válido)
-- O user_id é extraído automaticamente do token, não pode ser falsificado
```

#### Retorno
`boolean` - `true` se débito bem-sucedido, `false` se créditos insuficientes ou usuário não autenticado.
...
```sql
-- APENAS para uso via service_role em Edge Functions
SELECT debit_generation_credit_admin('user-uuid', 1);
```

#### Parâmetros
| Nome | Tipo | Default | Descrição |
|------|------|---------|-----------|
| `p_user_id` | UUID | - | ID do usuário (obrigatório) |
| `p_amount` | integer | 1 | Quantidade de créditos a debitar |

#### Retorno
`boolean` - `true` se débito bem-sucedido, `false` se créditos insuficientes.

**Nota de Segurança:** Esta função só pode ser executada por `service_role`. Usar em Edge Functions onde o `user_id` vem de uma fonte confiável (ex: tabela `image_generation_queue`).
