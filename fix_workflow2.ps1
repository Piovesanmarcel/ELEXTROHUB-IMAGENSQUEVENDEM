    ...
    if ($node.name -eq 'IF — "Validar Job Simples"' -and $node.parameters.conditions) {
        foreach ($cond in $node.parameters.conditions.conditions) {
            if ($cond.leftValue -like "*propertyName.jobId*") {
                Write-Host "[FIX] Corrigindo IF Validar Job Simples: propertyName.jobId -> jobId"
...
# Correcao 2: Conectar Schedule Trigger1 ao Redis SET TTL do lock
# Verificar se Schedule Trigger1 existe nas conexoes
if (-not $workflow.connections.'Schedule Trigger1') {
    Write-Host "[FIX] Criando conexao Schedule Trigger1..."
    $workflow.connections | Add-Member -NotePropertyName 'Schedule Trigger1' -NotePropertyValue @{
        main = @(@(@{node="Redis SET TTL do lock"; type="main"; index=0}))
    }
    $fixes++
} else {
    $conn = $workflow.connections.'Schedule Trigger1'.main
    if (-not $conn -or $conn.Count -eq 0 -or $conn[0].Count -eq 0) {
        Write-Host "[FIX] Conectando Schedule Trigger1 -> Redis SET TTL do lock"
        $workflow.connections.'Schedule Trigger1'.main = @(@([PSCustomObject]@{node="Redis SET TTL do lock"; type="main"; index=0}))
        $fixes++
    }
}
...
if ($fixes -eq 0) {
    Write-Host "[OK] Nenhuma correcao adicional necessaria"
    exit 0
}
